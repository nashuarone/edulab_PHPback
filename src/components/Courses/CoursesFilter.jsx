...
const CoursesFilter = (props) => {
  ...
  const currentTheme = useSelector((s) => s.coursesPage.currentTheme);
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(getFilteredCourses(themeId));
  }, [dispatch, themeId, coursesData.length]);
  useEffect(() => {
    dispatch(getCurrentTheme(themeId));
  }, [dispatch, themeId]);
  return (
    <div className={s.coursesPage}>
      <div className={s.editBlockTitle}>
        Курсы по направлению {currentTheme.title}
      </div>
      <NavLink to={"/themes"}>
      ...
      <div>
        {coursesData.length > 0
              ...
              <div className={s.courseItem} key={c.id}>
                ...
                <div>
                  <button
                    ...
                    onClick={() =>
                      dispatch(addUserCourse(c.id, currentLearnerId))
                    }
                  >
                    Записаться на курс
                  </button>
                ...
                {isTeacher && (
                  <div>
                    <button
                      className={s.courseBtn}
                      onClick={() =>
                        dispatch(deleteCourse(c.id, currentLearnerId))
                      }
                    >
                      Удалить курс
                    </button>
          ...
          : "За текущим направлением курсы не закреплены"}
      </div>
    </div>
  );
};

export default compose(withRouter)(CoursesFilter);
