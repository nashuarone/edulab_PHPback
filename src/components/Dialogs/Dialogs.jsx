...
const Dialogs = (props) => {
  ...
  const phoneMask = (target) => {
    window.addEventListener("DOMContentLoaded", function () {
      [].forEach.call(document.querySelectorAll("#antoxa"), function (input) {
        var keyCode;
        function mask(event) {
  ...
  return (
    <div className={s.dialogs}>
      ...
      <div className={s.messages}>
        ...
        <div>
          <input value={xxx} ref={inputRef} onChange={phoneMask} id="antoxa" placeholder="Номер сохраняется в БД"></input>
        </div>
        <div>
          <TestMask />
        </div>
